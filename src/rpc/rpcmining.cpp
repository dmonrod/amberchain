...
/* AMB START */
/*void loginvalidblock(const CBlockIndex* pindex, const CWallet pwalletMain)
{
    CPubKey miner  = pindex->kMiner;
    CPubKey defaultKey = pwalletMain->vchDefaultKey;
    CBitcoinAddress minerAddress(miner.GetID());    
    CBitcoinAddress defaultAddressKey(defaultKey.GetID());

    Object data;
    data.push_back(Pair("block-hash",pindex->GetBlockHash().ToString()));
    data.push_back(Pair("height", to_string((pindex->nHeight))));
    data.push_back(Pair("log2-work", to_string(log(pindex->nChainWork.getdouble())/log(2.0))));
    ...
    Array params;
    params.push_back(defaultAddressKey.ToString());
    params.push_back(STREAM_INVALIDBLOCKS);
    params.push_back(minerAddress.ToString());
    params.push_back(hex_data);

    publishfrom(params, false);
}*/
/* AMB END */